...
// eurocrypt
u16 ID[1][2] = {{13, 6}}, OD[3][2] = {{2, 0}, {7, 7}, {8, 0}};
...
int main(int argc, char *argv[])
{
  ...
  dparams.samplesPerloop = pow(2, 10);
  ...
  while (loop < dparams.totalLoopCount)
  {
    ...
    // fwdBit = state.nDifferenceBit(DiffState, OD, 3);
    ...
    // bwdBit = state.nDifferenceBit(DiffState, OD, 3);
    // epsilon_a find
    if (fwdBit == bwdBit)
      count++;
    loop++;
