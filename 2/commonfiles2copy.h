...
namespace Parameters
{
    ...
    struct diffDetailstrct
    {
        ...
        size_t IDrow, ODrow;
        bool halfroundflag;
        diffDetailstrct(u16 fwdround = 0, u16 (*ID)[2] = nullptr, u16 (*OD)[2] = nullptr, bool halfroundflag = false, size_t IDrow = 0, size_t ODrow = 0) : fwdround(fwdround), ID(ID), OD(OD), halfroundflag(halfroundflag), IDrow(IDrow), ODrow(ODrow) {}
    } diffdetails;
...
public:
    ...
    void chacha(u32 *x, bool flag = true, u32 value = 0)
    {
        if (flag)
        {
            x[0] = 0x61707865;
