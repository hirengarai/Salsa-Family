...
uint32_t IndexMap_iIV(int iIV, uint32_t Vnonfree[SIZE_WORD], uint32_t dimVVnonfree)
{ // s1
    uint32_t IV = 0;
    for (int i = 0; i < dimVVnonfree - 1; i++)
        IV |= (uint32_t)(GET_BIT(iIV, i)) << Vnonfree[i];
...
void Verification()
{
    ...
    for (int i = 0; i < dimVVnonfree_temp - 1; i++)
        Vnonfree_temp[i] = Vnonfree_theoretical[i];
    ...
    for (uint64_t iKey = 0; iKey < NK; iKey++)
        allKeys_temp[iKey] = new uint32_t[2];
    for (uint64_t iKey = 0; iKey < NK; iKey++)
    {
