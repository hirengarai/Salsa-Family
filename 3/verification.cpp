...
int cut_condition(int j, int n)
{ // n: numSyncopatedSegments
...
double GetMedianABS(double *array, uint64_t NS)
{
    ...
    if (NS % 2 == 1)
        ...
        return (0.5 * (fabs(array[NS / 2]) + fabs(array[NS / 2 - 1])));
}
...
void InvertLastRound_theoretical(int flagSYNC, uint32_t X_internal[16], uint32_t XX_internal[16], uint32_t X_end[16], uint32_t XX_end[16], uint32_t initX[16], uint32_t Z[16], int numSyncopatedSegments, int syncopatedSegments[SIZE_KEY][2])
{
    ...
    if (flagSYNC == 1)
        UpdateSyncopatedSegments(0, XX_end, X_end, initX, ZZ, numSyncopatedSegments, syncopatedSegments);
    ...
    if (flagSYNC == 1)
        UpdateSyncopatedSegments(1, XX_tmp, X_tmp, initX, ZZ, numSyncopatedSegments, syncopatedSegments);

    //"inverse" one round at the last second branch, for ChaCha7
    for (i = 8; i < 12; i++)
        ZZ[i] = Z[i] - XX_end[i + 4] - XX_tmp[i + 4];
    // Odd half round, for ChaCha7
    ...
    for (i = 0; i < 16; i++)
        XX_internal[i] = XX_tmp[i];

    /*<2. update the values of syncopated segments according to the syncopation technique, for ChaCha7*/
    if (flagSYNC == 1)
        UpdateSyncopatedSegments(2, XX_internal, X_internal, initX, ZZ, numSyncopatedSegments, syncopatedSegments);
}
double EstimateApproxCPNBs_single_theoretical(double numIVsExp, double num_target, double num_diff, int indexOD, int arrayPNBs[SIZE_KEY], int numPNBs, int syncopatedSegments[SIZE_KEY][2], int numSyncopatedSegments)
{
    ...
    NK = ((uint64_t)1 << 10);
    if (detailedInfos == 1)
        printf("	- number of keys: 2^{%.0f}\n", log(NK) / log(2));
    NIV = (uint64_t)pow(2, numIVsExp);
    if (detailedInfos == 1)
        printf("	- number of IVs: 2^{%.1f}\n", log(NIV) / log(2));
...
int CheckRestrictedConditions(uint32_t Z[16], uint32_t X[16], int numSyncopatedSegments, int indexRestrictedSegments[SIZE_KEY][2])
{
    ...
    for (w = 0; w < 4; w++)
        X_internal[12 + w] = ROTL(X_end[12 + w], 24) ^ X_end[w];
    ...
    for (int i = 0; i < numSyncopatedSegments; i++)
    {
        ...
        // ++++++++ cutting down the number of conditions   ++++++++
        if (valueX == valueZ && cut_condition(i, wt_cut) != 1)
        { // condition with type 0
            res = 0;
            return res;
        }
        // ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        ...
        if (pWord >= 8)
        {
            ...
            // ++++++++ cutting down the number of conditions   ++++++++
            if (valueX == valueZ && cut_condition(i + pos_gap, wt_cut) != 1)
            { // condition with type 1
                res = 0;
                return res;
            }
            // ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            ...
            // ++++++++ cutting down the number of conditions   ++++++++
            if (valueX == valueZ && cut_condition(i + 2 * pos_gap, wt_cut) != 1)
            { // condition with type 2
                res = 0;
                return res;
            }
            // ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
double EstimateApproxCPNBs_single(double numIVsExp, double num_target, double num_diff, int indexOD, int arrayPNBs[SIZE_KEY], int numPNBs, int numSyncopatedSegments, int theta[2], int indexRestrictedSegments[SIZE_KEY][2])
{
    ...
    if (flagSYNC == 1)
        ratioFiltered = pow(0.5, theta[1] - wt_cut) * pow(0.75, theta[0]);
    else
        ratioFiltered = 1.0;
    if (detailedInfos == 1)
        printf("		-- ratio of filtering: 2^{-%.4lf}, ", -log(ratioFiltered) / log(2));
    double NUM_T_filtered_expected = (double)(NIV)*ratioFiltered;
    if (detailedInfos == 1)
        printf("expected number of IVs: 2^{%.4lf}\n", log(NUM_T_filtered_expected) / log(2));
    ...
    for (uint64_t iKey = 0; iKey < NK; iKey++)
    {
        ...
        for (uint64_t iIV = 0; iIV < NIV; iIV++)
        {
            ...
            int flagRestrictedC;
            if (flagSYNC == 1)
                flagRestrictedC = CheckRestrictedConditions(Z, X.input, numSyncopatedSegments, indexRestrictedSegments);
            else
                flagRestrictedC = 1;
...
void Verification_BackApprox(double rounds_target, double rounds_diff, int OD)
{
    ...
    for (i = 0; i < SIZE_KEY; i++)
        signCPNB_final[i] = 0;
    for (i = 0; i < numCPNBs_final; i++)
        signCPNB_final[arrayCPNBs_final[i]] = 1;
...
void AttackOnChaCha7()
{
    ...
    assert((flag74or89 == 0) | (flag74or89 == 1));
    if (flag74or89 == 0)
        printf("	With 74 CPNBs,\n\n");
    else
        printf("	With 89 CPNBs,\n\n");
