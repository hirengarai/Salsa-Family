...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < 80; i++)
        PNB[i] = -1;
    for (int PNBindex{0}; PNBindex < 80; ++PNBindex)
    {
        ...
        for (int i{0}; i < max_num_threads; ++i)
            threadPool[i] = thread(&findPNB, PNBindex);
        ...
        for (int i{0}; i < 256; ++i)
        {
            double nm = 2 * (array_of_nm[i] / sampledetails.samplesperLoop) - 1.0;
            // std::cout << i << " - " << array_of_nm[i] << " || ";
            if (nm > max_bias)
            {
                max_index = i;
                max_bias = nm;
...
void findPNB(int PNBcount)
{
    u32 x0[WORD_COUNT], strdx0[WORD_COUNT], key[KEY_COUNT], dx0[WORD_COUNT], dstrdx0[WORD_COUNT], z[WORD_COUNT], dz[WORD_COUNT], DiffState[WORD_COUNT], threadloop{0};
    u16 fwdBit, bwdBit;
    ...
    while (threadloop < sampledetails.samplesperThread)
    {
        ...
        {
            if ((PNB[i] != -1) && GenerateRandomBoolean())
            {
        ...
        for (int key{0}; key < 256; ++key)
        {
            ...
            else
            {
                ...
                int WORD = (key / WORD_SIZE) + 4;
                int BIT = key % WORD_SIZE;
                bool flipflag = false;
