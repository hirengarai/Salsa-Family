...
int main()
{
    ...
    srand48(time(NULL));
    u32 x0[16], dx0[16],diffstate[16], strdx0[16], key[8];
    ...
    x0[15] = 0x00000000;
    CopyState(dx0, x0, 16);
    InputDifference(dx0, 13, 6);
    XORDifference(diffstate, 16, x0, dx0);

    cout << "Initial diff. State: \n";
    ...
    for (int i{1}; i <= 1; ++i)
        fwd.XRoundFunction(diffstate, i);

    cout << "State after " << 1 << " rounds: \n";
    PrintState(prntst);
    setbitscount.x = diffstate;
    setbitscount.columnno = 2;
    cout <<classsetbitscounter.FunSetBitsCount(setbitscount)<<"\n";

    // for (int i{dParams.totalround}; i > 0; i--)
    //     bwd.RoundFunction(x0, i);

    // cout << "State after " << dParams.totalround << " rev. rounds: \n";
    // PrintState(prntst);
}
