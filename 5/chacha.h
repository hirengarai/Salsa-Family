...
{
public:
  void QuarterRound(u32 *x, u32 a, u32 b, u32 c, u32 d)
  {
    a += b;
    d = ROTATE_LEFT(a ^ d, 16);
    c += d;
  ...
  void XRoundFunction(u32 *x, u32 round)
  {
    if (round & 1)
    {
      QuarterRound(x, 0, round % 4 * 4, round % 4 * 4 + 1, round % 4 * 4 + 2);
      QuarterRound(x, 3, round % 4 * 4 + 5, round % 4 * 4 + 6, round % 4 * 4 + 7);
      QuarterRound(x, 2, round % 4 * 4 + 10, round % 4 * 4 + 11, round % 4 * 4 + 8);
      QuarterRound(x, 1, round % 4 * 4 + 15, round % 4 * 4 + 12, round % 4 * 4 + 13);
    }
    else
    {
      QuarterRound(x, 0, round % 4 * 4, round % 4 * 4 + 5, round % 4 * 4 + 10);
      QuarterRound(x, 1, round % 4 * 4 + 1, round % 4 * 4 + 6, round % 4 * 4 + 11);
      QuarterRound(x, 2, round % 4 * 4 + 2, round % 4 * 4 + 7, round % 4 * 4 + 8);
      QuarterRound(x, 3, round % 4 * 4 + 3, round % 4 * 4 + 4, round % 4 * 4 + 9);
    }
  }

  void HalfRound(u32 *x, u32 round, bool isEven)
  {
    if (isEven)
    {
      QuarterRound(x, 0, round % 4 * 4, round % 4 * 4 + 1, round % 4 * 4 + 2);
  ...
  void RoundFunction(u32 *x, u32 round)
  {
    XRoundFunction(x, round);
    HalfRound(x, round, true);
    HalfRound(x, round, false);
  }

  // Quarter round function for standalone use
  void SingleQuarterRound(u32 &a, u32 &b, u32 &c, u32 &d)
  {
    a += b;
    d = ROTATE_LEFT(a ^ d, 16);
    c += d;
