 ...
 * running command: g++ syncopnb.cpp -pthread && ./a.out
 */
...
u32 ITERATION = pow(2, 20);
double THRESHOLD = pow(2.0, -1); // the measure threshold that the PNB will not affect the output of cipher
...
int main()
{
    // ---------------------------FILE CREATION------------------------------------------------------------------------------
    char filename[25];
    sprintf(filename, "%s%isyncopnb%i_%i.txt", cipher, totalround, IDword[0], IDbit[0]);
